...
- 双路线并行(CLAUDE.md 约定):**路线 A** 男声库(说教感占优,RVC 变调
  负担看音域差);**路线 B** 接近东雪莲音域的女声库(RVC transpose≈0 优先)。
...
- 全局参数起点:vibrato depth 全局先压到 ~0.6 倍(东雪莲原声 vibrato 不重,
  RVC 会保留源的 pitch 曲线);loudness 曲线后期不画,交给混音。
...
- **行 23 拆音(唯一改动过音符数的行)**:对照 mp3 找「づ」「な」中时值较长者
  ...
  否则改拆另一个;k 辅音务必清晰(Phase 3 RVC protect 参数也靠它)。
